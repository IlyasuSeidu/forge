...
import { getVerification, getAppRequest } from '@/lib/api/agents';
...
export default function VerificationExecutorPage() {
  ...
  const { currentState } = useAgentState('verification-executor');
  ...
  const [realVerification, setRealVerification] = useState<any>(null);
  const [appRequest, setAppRequest] = useState<any>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        setIsLoading(true);
        // First get app request to get the appRequestId
        // In a real scenario, we'd pass this from the route or context
        // For now, we'll use a placeholder approach
        // TODO: Pass appRequestId from route params or context

        // This is a placeholder - in production, the appRequestId would come from route
        const mockAppRequestId = 'app-request-1';

        const [verificationData, appRequestData] = await Promise.all([
          getVerification(projectId, mockAppRequestId),
          getAppRequest(projectId, mockAppRequestId).catch(() => null)
        ]);

        setRealVerification(verificationData);
        setAppRequest(appRequestData);
      } catch (err) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Real Verification Status (from backend API) */}
      <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-blue-900 mb-1">Current Verification System Status</h3>
            <p className="text-sm text-blue-800">
              This displays real data from the backend API. The detailed step-by-step results shown below are from the future "hardened" implementation.
            </p>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-lg border border-blue-200 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                realVerification.status === 'passed'
                  ? 'bg-green-100 text-green-800'
                  : realVerification.status === 'failed'
                  ? 'bg-red-100 text-red-800'
                  : 'bg-amber-100 text-amber-800'
              }`}>
                {realVerification.status.toUpperCase()}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700">Verification ID:</span>
              <span className="text-xs font-mono text-gray-600">{realVerification.id}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700">Attempt:</span>
              <span className="text-sm text-gray-900">{realVerification.attempt}</span>
            </div>
            {realVerification.errors && realVerification.errors.length > 0 && (
              <div className="pt-3 border-t border-blue-100">
                <div className="text-sm font-medium text-gray-700 mb-2">Errors ({realVerification.errors.length}):</div>
                <div className="bg-red-50 border border-red-200 rounded p-3 space-y-1">
                  {realVerification.errors.slice(0, 3).map((err: string, idx: number) => (
                    <div key={idx} className="text-xs text-red-900 font-mono">{err}</div>
                  ))}
                  {realVerification.errors.length > 3 && (
                    <div className="text-xs text-red-700 italic">... and {realVerification.errors.length - 3} more errors</div>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <p className="text-sm text-gray-600">No verification data available yet.</p>
          </div>
        )}
      </div>
      ...
      {/* Part 3: Generated Artifact - Verification Results (Future Hardened Implementation) */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="mb-4 bg-purple-50 border border-purple-200 rounded-lg p-4">
          <div className="flex items-start gap-2">
            ...
            <div>
              <h4 className="text-sm font-semibold text-purple-900 mb-1">Future Implementation Preview</h4>
              <p className="text-xs text-purple-800">
                The detailed step-by-step results below show the planned "hardened" Verification Executor with hash-locked contracts and deterministic step execution.
                This is currently using mock data. The backend has VerificationResult and VerificationReport database models ready, but REST endpoints are not yet implemented.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-900">Verification Results (Mock Data)</h3>
